...
class DegreeSoughtModel(Base):
    __tablename__ = "degree_sought"
    ...
    user_profiles = relationship('ProfileModel', back_populates="degree_soughts")
    
    #define the static methods
    @staticmethod
    def get_degree_object(db: Session):
        return db.query(DegreeSoughtModel)
    
    # get degree by ID
    @staticmethod
    def get_degree_by_id(db: Session, id: int):
        return DegreeSoughtModel.get_degree_object(db).get(id)
    
    # get degree by slug
    @staticmethod
    def get_degree_by_slug(db:Session, slug: str):
        return DegreeSoughtModel.get_degree_object(db).filter_by(slug=slug).first()

    # get degree by name
    @staticmethod
    def get_degree_by_name(db:Session, name: str):
        return DegreeSoughtModel.get_degree_object(db).filter_by(degree_name=name).first()
    
    # get all degrees
    @staticmethod
    def get_all_degrees(db: Session):
        return DegreeSoughtModel.get_degree_object(db)